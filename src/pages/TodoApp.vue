...
const newItem = () => {
  if (addnewtask.value) {
    tasks.value.push({ name: addnewtask.value, id: id.value > id.value + 1 });
  }
  addnewtask.value = "";
};
const delItem = (index) => {
...
const totalTask = computed(() => {
  const totalTaskcount = tasks.value.length;
  return totalTaskcount;
});
const completedTask = computed(() => {
  const totalcompletedTask = tasks.value.filter(
    (x) => x.checked === true
  ).length;
  return totalcompletedTask;
});
const uncompletedTask = computed(() => {
  const totalUncompletedTask =
    parseFloat(totalTask.value) - parseFloat(completedTask.value);
  return totalUncompletedTask;
});
</script>
